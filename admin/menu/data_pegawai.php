...
<head>
    ...
    <title>penjualan-Data pegawai</title>
</head>
        ...
        <tbody>
            ...
            if ($_POST['cari']) {
                if ($inputan == "") {
                    ...
                    // var_dump($inputan);
                    // die;
                    $q = mysqli_query($koneksi, "SELECT * FROM tb_kasir WHERE nama LIKE '%$inputan%' OR alamat LIKE '%$inputan%' OR telepon LIKE '%$inputan%' OR akses LIKE '%$inputan%' OR username LIKE '%$inputan%'");
                }
            } else {
                $q = mysqli_query($koneksi, "SELECT * FROM tb_kasir WHERE akses='kasir'");
            }
            ...
            if ($cek < 1) {
                ...
                <?php
                    ...
                    <tr>
                        ...
                        <td><?= $data['status']; ?></td>
                        <td>
                            <a onclick="return confirm('Anda yakin akan menghapusnya?')" href="?menu=hapus_pegawai&id_pegawai=<?php echo $data['id_kasir']; ?>"><span class=" glyphicon glyphicon-trash" aria-hidden="true"></span></a>
                            |
                            <a href="?menu=edit_pegawai&id_pegawai=<?php echo $data['id_kasir']; ?>"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
                        </td>
                    </tr>
            <?php
                }
